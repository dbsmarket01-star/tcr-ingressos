...
describe("Asaas split calculation", () => {
  ...
  it("pays the fixed split for every admission generated by a double ticket", () => {
    expect(
      calculateAsaasSplitsForOrder(
        [{ quantity: 1, admissionsPerUnit: 2, totalInCents: 19790 }],
        rules
      )
    ).toEqual([
      { walletId: "diego", fixedValue: 15.83 },
      { walletId: "pietro", fixedValue: 15.83 },
      { walletId: "lucas", fixedValue: 3 }
    ]);
  });
});
