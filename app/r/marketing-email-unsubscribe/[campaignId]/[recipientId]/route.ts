...
export async function GET(
  ...
  if (!recipient) {
    return new Response(
      renderHtml("Não encontramos esse vínculo de campanha. Fale com o organizador para revisar seu cadastro.", false),
      {
        headers: {
  ...
  const changed = !recipient.emailOptOutAt;
  ...
  return new Response(
    renderHtml(
      changed
        ? "Seu e-mail foi retirado dos próximos disparos desta campanha."
        : "Seu e-mail já estava descadastrado para esta campanha.",
      changed
    ),
    {
      headers: {
