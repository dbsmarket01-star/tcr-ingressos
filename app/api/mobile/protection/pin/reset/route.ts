...
import { authorizePinReset, getAppUserFromBearerToken } from "@/features/mobile-auth/mobile-auth.service";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!unlock.canProceed || !unlock.request) {
      return NextResponse.json({ error: "Reset de PIN exige aprovação supervisionada." }, { status: 403 });
    }

    const result = await authorizePinReset({
      userId: auth.user.id,
      deviceId: parsed.data.deviceId
    });
    ...
    return NextResponse.json(
      { ok: true, resetAuthorizedAt: result.resetAuthorizedAt },
      { headers: { "Cache-Control": "no-store" } }
    );
  } catch (error) {
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Nao foi possivel autorizar o reset do PIN." },
      { status: 400 }
    );
  }
}
