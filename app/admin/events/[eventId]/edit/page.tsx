...
export default async function EditEventPage({ params }: EditEventPageProps) {
  ...
  return (
    ...
    >
      ...
      <section className="card form wideForm spacedSection">
        <div className="formSectionHeader">
          <div>
            <span className="sectionEyebrow">Ingressos</span>
            <h2>Lotes e hotelaria</h2>
          </div>
          <p className="muted">
            Crie novos ingressos e marque hotel apenas quando a compra precisar entrar na HOME LIST.
          </p>
        </div>
              ...
              <tbody>
                {event.lots.map((lot) => (
                  <tr key={lot.id}>
                    ...
                    <td>{lot.hasHotel ? "Possui hotel" : "Sem hotel"}</td>
                    <td>
        ...
        <form action={createTicketLotAction} className="formSection lotCreateForm" id="criar-ingresso">
          <div className="formSectionHeader">
            ...
            <p className="muted">Depois de criar, você pode editar detalhes avançados quando quiser.</p>
          </div>
          <input type="hidden" name="eventId" value={event.id} />
          <div className="grid twoColumns">
            <label className="field">
